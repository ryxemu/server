...
void Client::SendLogServer() {
	...
	const char *wsn = Config::get()->WorldShortName.c_str();
...
bool Client::HandleGenerateRandomNamePacket(const EQApplicationPacket *app) {
	...
	bool vwl = false;
	bool dbl = false;
	if (rndnum > 63) {               // rndnum is 0 - 75 where 64-75 is cons pair, 17-63 is cons, 0-16 is vowel
	...
	for (int i = n; i < namlen; i++) {
		...
		vwl = !vwl;
		if (!dbl && !dlc) {                 // one chance at double letters in name
	...
	if (!pZoning) {
		...
		if (raidid > 0) {
			...
			std::string query = StringFormat(
			    "SELECT groupid, isgroupleader, israidleader, islooter "
	...
	if (cle && cle->IsLocalClient()) {
		...
		if (local_addr[0]) {
			...
			if (strcmp(zs_addr, "127.0.0.1") == 0) {
				Log(Logs::Detail, Logs::WorldServer, "Local zone address was %s, setting local address to: %s", zs_addr, Config::get()->WorldLANIP.c_str());
				zs_addr = Config::get()->WorldLANIP.c_str();
			} else {
		...
		if (addr[0]) {
			...
			zs_addr = Config::get()->WorldWANIP.c_str();
		}
	}
