...
void destroy_range(FwdIter first, FwdIter last) {
	while (first != last)
		delete *first++;
}
...
///
bool Suite::run(Output& output, bool cont_after_fail) {
...
///             equal \e class::func.
///
void Suite::register_test(Func func, const string& name) {
...
///
void Suite::assertment(Source s) {
	s._suite = _name;
	s._test = *_cur_test;
...
//
void Suite::do_run(Output* os, bool cont_after_fail) {
...
// its embedded suites.
//
Time Suite::total_time(bool recursive) const {
	Time time = accumulate(_tests.begin(), _tests.end(),
	                       Time(), SuiteTime());

	return !recursive ? time : accumulate(_suites.begin(), _suites.end(), time, SubSuiteTime());
}
