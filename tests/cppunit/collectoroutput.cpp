...
void CollectorOutput::finished(int tests, const Time& time) {
	_total_tests = tests;
	_total_time = time;
}

void CollectorOutput::suite_start(int tests, const string& name) {
	if (tests > 0) {
...
void CollectorOutput::suite_end(int tests, const string&, const Time& time) {
	if (tests > 0) {
...
void CollectorOutput::test_start(const string& name) {
	_cur_suite->_tests.push_back(TestInfo(name));
	_cur_test = &_cur_suite->_tests.back();
}

void CollectorOutput::test_end(const string&, bool ok, const Time& time) {
	if (!(_cur_test->_success = ok))
		++_cur_suite->_errors;
	_cur_test->_time = time;
}

void CollectorOutput::assertment(const Source& s) {
