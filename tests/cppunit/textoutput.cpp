...
void TextOutput::finished(int tests, const Time& time) {
	_stream << "Total: " << tests << " tests, "
	        << correct(tests, _total_errors) << "% correct"
	        << " in " << time << " seconds" << endl;
}

void TextOutput::suite_start(int tests, const string& name) {
	if (tests > 0) {
		_suite_name = name;
...
void TextOutput::suite_end(int tests, const string& name, const Time& time) {
	if (tests > 0) {
...
void TextOutput::test_end(const string&, bool ok, const Time&) {
	_stream << _suite_name << ": "
...
void TextOutput::assertment(const Source& s) {
