...
Pet::Pet(NPCType *type_data, Mob *owner, PetType type, uint16 spell_id, int16 power, int16 focusItemId)
	...
	if (typeofpet == petHatelist) {
		...
		if (tar && DistanceSquaredNoZ(owner->GetPosition(), tar->GetPosition()) < (200 * 200) && IsAttackAllowed(tar) && (tar->GetZ() - owner->GetZ() < 75) && (tar->GetZ() - owner->GetZ() > -75))
			AddToHateList(tar, 1);
...
bool ZoneDatabase::GetPoweredPetEntry(const char *pet_type, int16 petpower, PetRecord *into) {
	...
	if (petpower <= 0)
		query = StringFormat(
		    "SELECT npcID, temp, petpower, petcontrol, petnaming, monsterflag, equipmentset "
		    "FROM pets WHERE type='%s' AND petpower<=0",
		    pet_type);
	else
		query = StringFormat(
		    "SELECT npcID, temp, petpower, petcontrol, petnaming, monsterflag, equipmentset "
