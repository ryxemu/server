...
bool Client::TradeskillExecute(DBTradeskillRecipe_Struct* spec) {
	...
	/* Trade skill Fail */
		...
		if (this->GetGroup()) {
			entity_list.MessageGroup(this, true, MT_Skills, "%s was unsuccessful in %s tradeskill attempt.", GetName(), this->GetGender() == 0 ? "his" : this->GetGender() == 1 ? "her"
			                                                                                                                                                                    : "its");
		}
...
bool ZoneDatabase::GetTradeRecipe(uint32 recipe_id, uint8 c_type, uint32 some_id,
	            ...
	            FROM
	                tradeskill_recipe
	                    INNER JOIN tradeskill_recipe_entries ON tradeskill_recipe.id = tradeskill_recipe_entries.recipe_id
	                                                                                       WHERE
	                                                                                           tradeskill_recipe.id = % lu AND tradeskill_recipe_entries.item_id % s AND tradeskill_recipe_entries.iscontainer = 1 AND tradeskill_recipe.enabled
	                                                                                                                                                                                                                 GROUP BY
	                                                                                                                                                                                                                     tradeskill_recipe.id),
	...
	query = StringFormat(
	    "SELECT item_id,successcount FROM tradeskill_recipe_entries "
...
bool ZoneDatabase::EnableRecipe(uint32 recipe_id) {
	std::string query = StringFormat(
	    "UPDATE tradeskill_recipe SET enabled = 1 "
	    "WHERE id = %u;",
	    recipe_id);
	auto results = QueryDatabase(query);
	if (!results.Success())
...
bool ZoneDatabase::DisableRecipe(uint32 recipe_id) {
	std::string query = StringFormat(
	    "UPDATE tradeskill_recipe SET enabled = 0 "
	    "WHERE id = %u;",
	    recipe_id);
	auto results = QueryDatabase(query);
	if (!results.Success())
