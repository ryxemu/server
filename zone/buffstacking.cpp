...
int Mob::FindAffectSlot(Mob *caster, uint16 spell_id, int *result_slotnum, int remove_replaced) {
	...
	*result_slotnum = -1;
	...
	if (new_spelldata->classes[BARD - 1] != 255 && caster->GetClass() == BARD)  // Bard casting bard song
	{
		for (int cur_slotnum2 = 0; cur_slotnum2 < this->GetMaxBuffSlots(); cur_slotnum2++) {
			...
			if (old_buff2->spellid != SPELL_UNKNOWN && BuffStacking::IsValidSpellIndex(old_buff2->spellid)) {
				...
				if (old_buff_spelldata2 && old_buff_spelldata2->classes[BARD - 1] == 255      // existing buff is not bard song
		...
		if (old_buff3->spellid != SPELL_UNKNOWN) {
			if (old_buff3->spellid == spell_id) {
				...
				OVERWRITE_SAME_SPELL_WITHOUT_REMOVING_FIRST:
					if (caster->GetLevel() >= old_buff3->casterlevel && GetSpellEffectIndex(new_spelldata->id, SE_EyeOfZomm) == -1 && GetSpellEffectIndex(new_spelldata->id, SE_CompleteHeal) == -1  // Donal's BP
	...
	{
		if (new_spelldata->goodEffect && GetSpellEffectIndex(new_spelldata->id, SE_MovementSpeed) != -1 && GetSpellEffectIndex(old_spelldata->id, SE_MovementSpeed) != -1 || new_spelldata->goodEffect && GetSpellEffectIndex(new_spelldata->id, SE_MovementSpeed) != -1 && GetSpellEffectIndex(old_spelldata->id, SE_Root) != -1) {
	...
	while (2) {
		...
		if ((old_buff_effectid == SE_CurrentMana || old_buff_effectid == SE_CurrentHP || old_buff_effectid == SE_HealOverTime) && old_spelldata->classes[BARD - 1] != 255 && new_spelldata->classes[BARD - 1] == 255  // old buff is a bard song and new buff is not a bard song
	...
	if (new_spelldata->goodEffect && (!old_spelldata->goodEffect || GetSpellEffectIndex(old_spelldata->id, SE_Illusion) != -1) || old_spelldata->effectid[effect_slot_num] == SE_CompleteHeal              // Donal's BP effect
	    || old_buff_spellid >= 775 && old_buff_spellid <= 785 || old_buff_spellid >= 1200 && old_buff_spellid <= 1250 || old_buff_spellid >= 1900 && old_buff_spellid <= 1924 || old_buff_spellid == 2079  // ShapeChange65
