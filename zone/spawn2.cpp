...
bool Spawn2::Process() {
	...
	if (timer.Check()) {
		...
		if (condition_id != SC_AlwaysEnabled && !zone->spawn_conditions.Check(condition_id, condition_min_value)) {
		...
		if (rand_spawn || (spawn_group->roamdist && open_outdoor_zone)) {
			...
			while (z == BEST_Z_INVALID && count < 3) {
				...
				glm::vec3 loc(x, y, 0);
				z = zone->MapFindBestZ(loc, nullptr);
				++count;
			}
...
void SpawnConditionManager::UpdateDBCondition(const char *zone_name, uint16 cond_id, int16 value) {
	std::string query = StringFormat(
	    "REPLACE INTO spawn_condition_values "
	    "(id, value, zone) "
	    "VALUES( %u, %u, '%s')",
...
bool SpawnConditionManager::LoadSpawnConditions(const char *zone_name) {
	...
	// load values
	query = StringFormat(
	    "SELECT id, value FROM spawn_condition_values "
	    "WHERE zone = '%s'",
	    zone_name);
	...
	// load spawn events
	query = StringFormat(
	    "SELECT id, cond_id, period, next_minute, next_hour, "
...
uint32 ZoneDatabase::GetSpawnTimeLeft(uint32 id) {
	std::string query = StringFormat(
	    "SELECT start, duration FROM respawn_times "
	    "WHERE id = %lu",
	    (unsigned long)id);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
