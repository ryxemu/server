...
void NPC::DescribeAggro(Client *towho, Mob *mob, bool verbose) {
	...
	if ((mob->IsClient() &&
	     (!mob->CastToClient()->Connected() || mob->CastToClient()->IsLD() || mob->CastToClient()->IsBecomeNPC() || mob->CastToClient()->GetGM()))) {
...
bool Mob::CheckWillAggro(Mob *mob, bool turn_mobs) {
	...
	if (t3 < 0)
		t3 = 0 - t3;
	if ((t1 > iAggroRange) || (t2 > iAggroRange) || (t3 > iAggroRange) || (mob->IsInvisible(this) || (ownr && ownr->IsInvisible(this))) || (IsMezzed()) || (mob == GetOwner())) {
		return (false);
	}
	...
	if (
	    ((GetLevel() >= 18) || (GetBodyType() == BT_Undead) || (CastToNPC()->IsAggroOnPC()) || (mob->IsClient() && mob->CastToClient()->IsSitting()) || (oos->GetLevelCon(GetLevel()) != CON_GREEN)) &&
	    ((fv == FACTION_SCOWLS || (fv == FACTION_THREATENINGLY && zone->random.Roll(THREATENINGLY_ARRGO_CHANCE))))) {
	...
	if (turn_mobs && mob->IsClient() && IsNPC() && dist2 < 600 && !IsEngaged() && !IsMoving() && GetAppearance() == eaStanding && fv < FACTION_THREATENINGLY && CastToNPC()->CanTalk() && !CastToNPC()->GetRefaceTimer()->Enabled() && zone->random.Roll(0.01)) {
		FaceTarget(mob);
...
bool Mob::CheckDragAggro(Mob *mob) {
	...
	if (t3 < 0)
		t3 = 0 - t3;
	if ((t1 > iAggroRange) || (t2 > iAggroRange) || (t3 > iAggroRange) || (mob->IsInvisible(this)) || (mob->IsClient() && (!mob->CastToClient()->Connected() || mob->CastToClient()->IsLD() || mob->CastToClient()->IsBecomeNPC() || mob->CastToClient()->GetGM()))) {
		return (false);
	}
...
bool EntityList::AICheckClientAggro(NPC *aggressor) {
	...
	for (auto it = client_list.begin(); it != client_list.end(); ++it) {
		...
		if (!aggressor->CheckAggro(client) && aggressor->CheckWillAggro(client)) {
			if (!engaged || !aggressor->GetTarget() || (proxAggro && DistanceSquared(aggressor->GetPosition(), client->GetPosition()) < tankDist)) {
...
bool EntityList::AICheckNPCAggro(NPC *aggressor) {
	...
	for (auto it = npc_list.begin(); it != npc_list.end(); ++it) {
		...
		if (!aggressor->CheckAggro(npc) && aggressor->CheckWillAggro(npc)) {
			if (!engaged || !aggressor->GetTarget() || (proxAggro && DistanceSquared(aggressor->GetPosition(), npc->GetPosition()) < tankDist)) {
...
void EntityList::AIYellForHelp(Mob *sender, Mob *attacker) {
	...
	for (auto it = npc_list.begin(); it != npc_list.end(); ++it) {
		...
		if (zone->GetNumAggroedNPCs() >= zone->GetPullLimit() && npc->GetAggroDeaggroTime() < 30000 && entity_list.GetTopHateCount(attacker) >= zone->GetPullLimit()) {
			continue;
		}
		...
		if (
		    npc != sender && npc != attacker && npc->GetClass() != MERCHANT && npc->GetPrimaryFaction() != 0 && DistanceSquared(npc->GetPosition(), sender->GetPosition()) <= r && ((!npc->IsPet()) || (npc->IsPet() && npc->GetOwner() && !npc->GetOwner()->IsClient() && npc->GetOwner() == sender))  // If we're a pet we don't react to any calls for help if our owner is a client or if our owner was not the one calling for help.
		) {
