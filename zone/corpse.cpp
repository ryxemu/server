...
Corpse::Corpse(NPC* in_npc, ItemList* in_itemlist, uint32 in_npctypeid, const NPCType** in_npctypedata, uint32 in_decaytime, bool is_client_pet)
    : Mob("Unnamed_Corpse",              // const char* in_name,
      ...
      corpse_rez_timer(0),
      corpse_delay_timer(RuleI(Corpse, UnlockTimer)),
	...
	if (IsEmpty() && in_decaytime > RuleI(Corpse, EmptyNPCCorpseDecayTimeMS)) {
	...
	if (Loot_Request_Type >= 2 || (Loot_Request_Type == 1 && client->Admin() >= AccountStatus::GMAdmin && client->GetGM())) {
		...
		if (Loot_Request_Type == 5) {
			...
			if (inst) {
				...
				client->Message(CC_Red, "Could not find item number %i to send!!", GetPlayerKillItem());
			}
...
void Corpse::UpdateEquipmentLight() {
	...
	for (auto iter = itemlist.begin(); iter != itemlist.end(); ++iter) {
		if (((*iter)->equip_slot < EQ::invslot::EQUIPMENT_BEGIN || (*iter)->equip_slot > EQ::invslot::EQUIPMENT_END)) {
			continue;
		}
	...
	for (auto iter = itemlist.begin(); iter != itemlist.end(); ++iter) {
		if ((*iter)->equip_slot < EQ::invslot::GENERAL_BEGIN || (*iter)->equip_slot > EQ::invslot::GENERAL_END) {
			continue;
		}
