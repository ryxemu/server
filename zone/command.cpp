...
void command_zone(Client *c, const Seperator *sep) {
	...
	auto zone_id = (sep->IsNumber(1) ? std::stoul(zone_identifier) : database.GetZoneID(zone_identifier));
...
void command_peqzone(Client *c, const Seperator *sep) {
	...
	if (
	    c->IsInvisible(c) || c->IsRooted() || c->IsStunned() || c->IsMezzed() || c->AutoAttackEnabled() || c->GetInvul()) {
		...
		if (sep->IsNumber(2)) {
			...
			c->Message(
			    ...
			    fmt::format(
			        ...
			        target->GetID(),
			        (
			            faction_name.empty() ? std::to_string(faction_id) : fmt::format("{} ({})", faction_name, faction_id)))
			        .c_str());
		} else {
			c->Message(CC_Default, "Usage: #ai faction [Faction ID] - Set an NPC's Faction ID");
		}
...
void command_delacct(Client *c, const Seperator *sep) {
	...
	else if (database.DeleteAccount(sep->arg[1]))
...
void command_setpass(Client *c, const Seperator *sep) {
	...
	c->Message(
	    ...
	    fmt::format(
	        ...
	        (
	            database.SetLocalPassword(account_id, sep->arg[2]) ? "successfully" : "failed"),
	        account_name)
	        .c_str());
}
...
void command_flymode(Client *c, const Seperator *sep) {
	...
	c->Message(
	    ...
	    fmt::format(
	        "Fly Mode for {} is now {} ({}).",
	        (
	            c == target ? "yourself" : fmt::format("{} ({})", target->GetCleanName(), target->GetID())),
...
void command_peekinv(Client *c, const Seperator *sep) {
	...
	if (strcasecmp(sep->arg[1], "all") == 0) {
		scopeWhere = (peekOutOfScope);
	} else if (strcasecmp(sep->arg[1], "worn") == 0) {
		scopeWhere |= peekWorn;
	} else if (strcasecmp(sep->arg[1], "inv") == 0) {
		scopeWhere |= peekInv;
	} else if (strcasecmp(sep->arg[1], "cursor") == 0) {
		scopeWhere |= peekCursor;
	} else if (strcasecmp(sep->arg[1], "bank") == 0) {
		scopeWhere |= peekBank;
	} else if (strcasecmp(sep->arg[1], "trade") == 0) {
		scopeWhere |= peekTrade;
	} else if (strcasecmp(sep->arg[1], "world") == 0) {
		scopeWhere |= peekWorld;
	}
	...
	for (int16 indexMain = EQ::invslot::GENERAL_BEGIN; (scopeWhere & peekInv) && (indexMain <= EQ::invslot::GENERAL_END); ++indexMain) {
		...
		c->Message((item_data == 0), fmt::format("InvSlot: {}, Item: {} ({}), Charges: {}",
		                                         indexMain, ((item_data == 0) ? 0 : item_data->ID), linker.GenerateLink().c_str(), ((item_data == 0) ? 0 : inst_main->GetCharges()))
		                                 .c_str());
		...
		for (uint8 indexSub = EQ::invbag::SLOT_BEGIN; inst_main && inst_main->IsClassBag() && (indexSub <= EQ::invbag::SLOT_END); ++indexSub) {
			...
			c->Message((item_data == nullptr), fmt::format("  InvBagSlot: {} (Slot #{}, Bag #{}), Item: {} ({}), Charges: {}",
			                                               EQ::InventoryProfile::CalcSlotId(indexMain, indexSub), indexMain, indexSub, ((item_data == nullptr) ? 0 : item_data->ID), linker.GenerateLink().c_str(), ((item_data == nullptr) ? 0 : inst_sub->GetCharges()))
			                                       .c_str());
		}
	}
	// money
	...
	if (scopeWhere & peekCursor) {
		if (targetClient->GetInv().CursorEmpty()) {
			...
			c->Message(CC_Default, fmt::format("CursorSlot: {}, Item: {} ({}), Charges: {}",
			                                   EQ::invslot::slotCursor, 0, linker.GenerateLink().c_str(), 0)
			                           .c_str());
		} else {
			int cursorDepth = 0;
			for (auto it = targetClient->GetInv().cursor_cbegin(); (it != targetClient->GetInv().cursor_cend()); ++it, ++cursorDepth) {
				...
				c->Message((item_data == 0), fmt::format("CursorSlot: {}, Depth: {}, Item: {} ({}), Charges: {}",
				                                         SlotCursor, cursorDepth, ((item_data == 0) ? 0 : item_data->ID), linker.GenerateLink().c_str(), ((item_data == 0) ? 0 : inst_main->GetCharges()))
				                                 .c_str());
				...
				for (uint8 indexSub = EQ::invbag::SLOT_BEGIN; (cursorDepth == 0) && inst_main && inst_main->IsClassBag() && (indexSub <= EQ::invbag::SLOT_END); ++indexSub) {
					...
					c->Message((item_data == nullptr), fmt::format("  CursorBagSlot: {} (Slot #{}, Bag #{}), Item: {} ({}), Charges: {}",
					                                               EQ::InventoryProfile::CalcSlotId(SlotCursor, indexSub), SlotCursor, indexSub, ((item_data == nullptr) ? 0 : item_data->ID), linker.GenerateLink().c_str(), ((item_data == nullptr) ? 0 : inst_sub->GetCharges()))
					                                       .c_str());
				}
			}
		}
		...
		if (objectTradeskill == nullptr) {
			...
			for (int16 indexMain = EQ::invslot::SLOT_BEGIN; indexMain < EQ::invtype::WORLD_SIZE; ++indexMain) {
				...
				c->Message((item_data == nullptr), fmt::format("WorldSlot: {}, Item: {} ({}), Charges: {}",
				                                               (EQ::invslot::WORLD_BEGIN + indexMain), ((item_data == nullptr) ? 0 : item_data->ID), linker.GenerateLink().c_str(), ((item_data == 0) ? 0 : inst_main->GetCharges()))
				                                       .c_str());
				...
				for (uint8 indexSub = EQ::invbag::SLOT_BEGIN; inst_main && inst_main->IsClassBag() && (indexSub <= EQ::invbag::SLOT_END); ++indexSub) {
					...
					c->Message((item_data == nullptr), fmt::format("  WorldBagSlot: {} (Slot #{}, Bag #{}), Item: {} ({}), Charges: {}",
					                                               INVALID_INDEX, indexMain, indexSub, ((item_data == 0) ? 0 : item_data->ID), linker.GenerateLink().c_str(), ((item_data == nullptr) ? 0 : inst_sub->GetCharges()))
					                                       .c_str());
				}
			}
		}
...
void command_stun(Client *c, const Seperator *sep) {
	...
	std::string stun_message = (duration ? fmt::format(
	                                           "You stunned {} for {}.",
	                                           (
	                                               c == target ? "yourself" : fmt::format("{} ({})", target->GetCleanName(), target->GetID())),
...
void command_bestz(Client *c, const Seperator *sep) {
	...
	if (zone->watermap == nullptr) {
		...
		switch (RegionType) {
			case RegionTypeNormal: {
				c->Message(CC_Default, "There is nothing special about the region you are in!");
				break;
			}
			case RegionTypeWater: {
				c->Message(CC_Default, "You/your target are in Water.");
				break;
			}
			case RegionTypeLava: {
				c->Message(CC_Default, "You/your target are in Lava.");
				break;
			}
			case RegionTypeVWater: {
				c->Message(CC_Default, "You/your target are in VWater (Icy Water?).");
				break;
			}
			case RegionTypePVP: {
				c->Message(CC_Default, "You/your target are in a pvp enabled area.");
				break;
			}
			case RegionTypeSlime: {
				c->Message(CC_Default, "You/your target are in slime.");
				break;
			}
			case RegionTypeIce: {
				c->Message(CC_Default, "You/your target are in ice.");
				break;
			}
			default:
				c->Message(CC_Default, "You/your target are in an unknown region type %d.", (int)RegionType);
		}
	}
}
...
void command_advnpcspawn(Client *c, const Seperator *sep) {
	...
	if (is_add_entry) {
		...
		uint32 variance = (sep->IsNumber(3) ? std::stoi(sep->arg[3]) : spawn2->GetVariance());
		std::string query = fmt::format(
...
void command_viewzoneloot(Client *c, const Seperator *sep) {
	...
	if (search_item_id != 0) {
		std::string drop_string = (loot_amount > 0 ? fmt::format(
		                                                 "dropping in {} {} ",
		                                                 loot_amount,
