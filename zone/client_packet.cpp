		...
		case CLIENT_CONNECTING: {
			if (ConnectingOpcodes.count(opcode) != 1) {
				...
				Log(Logs::General, Logs::Error,
				    "HandlePacket() Opcode error: Unexpected packet during CLIENT_CONNECTING: opcode:"
				    " %s (#%d eq=0x%04x), size: %i",
				    OpcodeNames[opcode], opcode, 0, app->size);
...
void Client::Handle_OP_Animation(const EQApplicationPacket *app) {
	if (app->size != sizeof(Animation_Struct)) {
		Log(Logs::General, Logs::Error,
		    "Received invalid sized "
		    "OP_Animation: got %d, expected %d",
		    app->size,
...
void Client::Handle_OP_ClientUpdate(const EQApplicationPacket *app) {
	...
	if (app->size != sizeof(SpawnPositionUpdate_Struct) && app->size != (sizeof(SpawnPositionUpdate_Struct) + 1)) {
	...
	if (ppu->spawn_id != GetID()) {
		...
		// check if the id is for a boat the player is controlling
			...
			if (!boat || !boat->IsBoat()) {  // if the boat ID is invalid, reset the id and abort
				BoatID = 0;
				return;
			}
			...
			return;  // if not a boat, do nothing
	}
...
void Client::Handle_OP_Emote(const EQApplicationPacket *app) {
	if (app->size != sizeof(Emote_Struct)) {
		Log(Logs::General, Logs::Error,
		    "Received invalid sized "
		    "OP_Emote: got %d, expected %d",
		    app->size, sizeof(Emote_Struct));
		DumpPacket(app);
		return;
	}
	...
	uint32 len_packet = sizeof(in->unknown01) + len_name + len_msg + 1;
...
void Client::Handle_OP_GMSearchCorpse(const EQApplicationPacket *app) {
	...
	std::string query = StringFormat(
	    "SELECT charname, zoneid, x, y, z, time_of_death, rezzed, IsBurried "
	...
	std::string popupText =
	    "<table><tr><td>Name</td><td>Zone</td><td>X</td><td>Y</td><td>Z</td><td>Date</td><td>"
...
void Client::Handle_OP_MoveItem(const EQApplicationPacket *app) {
	...
	if (mi->from_slot >= EQ::invbag::GENERAL_BAGS_BEGIN && mi->from_slot <= EQ::invbag::CURSOR_BAG_END) {
		if (mi->from_slot >= EQ::invbag::CURSOR_BAG_BEGIN) {
			mi_hack = true;
		} else {
			int16 from_parent = m_inv.CalcSlotId(mi->from_slot);
			if (!m_inv[from_parent]) {
				mi_hack = true;
			} else if (!m_inv[from_parent]->IsType(EQ::item::ItemClassBag)) {
				mi_hack = true;
			} else if (m_inv.CalcBagIdx(mi->from_slot) >= m_inv[from_parent]->GetItem()->BagSlots) {
				mi_hack = true;
			}
		}
	}
	...
	if (mi->to_slot >= EQ::invbag::GENERAL_BAGS_BEGIN && mi->to_slot <= EQ::invbag::CURSOR_BAG_END) {
		if (mi->to_slot >= EQ::invbag::CURSOR_BAG_BEGIN) {
			mi_hack = true;
		} else {
			int16 to_parent = m_inv.CalcSlotId(mi->to_slot);
			if (!m_inv[to_parent]) {
				mi_hack = true;
			} else if (!m_inv[to_parent]->IsType(EQ::item::ItemClassBag)) {
				mi_hack = true;
			} else if (m_inv.CalcBagIdx(mi->to_slot) >= m_inv[to_parent]->GetItem()->BagSlots) {
				mi_hack = true;
			}
		}
	}
	...
	if (mi_hack) {
		Message(CC_Yellow, "Caution: Illegal use of inaccessable bag slots!");
	}
...
void Client::Handle_OP_RandomReq(const EQApplicationPacket *app) {
	...
	if (randLow == 0 && randHigh == 0) {  // defaults
		randLow = 0;
		randHigh = 100;
	}
...
void Client::Handle_OP_SetRunMode(const EQApplicationPacket *app) {
	if (app->size != sizeof(SetRunMode_Struct)) {
		Log(Logs::General, Logs::Error,
		    "Received invalid sized "
		    "OP_SetRunMode: got %d, expected %d",
		    app->size,
...
void Client::Handle_OP_SetServerFilter(const EQApplicationPacket *app) {
	if (app->size != sizeof(SetServerFilter_Struct)) {
		Log(Logs::General, Logs::Error,
		    "Received invalid sized "
		    "OP_SetServerFilter: got %d, expected %d",
		    app->size,
...
void Client::Handle_OP_TradeRequest(const EQApplicationPacket *app) {
	...
	if (tradee && tradee->IsClient()) {
		...
		if (tradee && ((tradee->CastToClient()->trade->state != TradeNone && tradee->CastToClient()->trade->GetWithID() != GetID()) || tradee->CastToClient()->IsFeigned())) {
