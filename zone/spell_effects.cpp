		...
		switch (effect) {
			...
			case SE_BindAffinity:  // TO DO: Add support for secondary and tertiary gate abilities
			{
				...
				if (IsClient()) {
					if (CastToClient()->GetGM() || RuleB(Character, BindAnywhere)) {
						...
						if (!zone->CanBind() || (GetZoneID() == kael && !zone->IsBindArea(GetX(), GetY(), GetZ())) || (GetZoneID() == skyshrine && !zone->IsBindArea(GetX(), GetY(), GetZ()))) {
			...
			case SE_Illusion: {
				...
				if (caster == this &&
				    ...
				    (spell_id != SPELL_MINOR_ILLUSION && spell_id != SPELL_ILLUSION_TREE))  // exclude these for permanent illusion AA
			...
			case SE_CurseCounter: {
				...
				if (effect_value < 0) {
					...
					for (int j = 0; j < buff_count; j++) {
						...
						// solar: there is an intentional weirdness here - the variable 'i' here is the slot number of the counter effect in the cure spell, but it's also used to index the buff spell.
						// it should probably use the index of the counter effect in the buff spell but this is how sony did it.
						if (effect_value + CalcSpellEffectValue(buffs[j].spellid, i, buffs[j].casterlevel, 0, buffs[j].instrumentmod) <= 6 || (buffs[j].spellid == SPELL_EPOCH_CONVICTION && spell_id == SPELL_REMOVE_GREATER_CURSE))  // TAKP customization for Quarm spell Epoch Conviction
						{
							buffs[j].counters += effect_value;
							if (buffs[j].counters <= 0) {
								if (caster)
	...
	*/
	...
	switch (formula) {
		...
		case 150:  // resistant discipline (custom formula)
			result = caster_level > 50 ? 10 : caster_level > 45 ? 5 + caster_level - 45
			                              : caster_level > 40   ? 5
			                              : caster_level > 34   ? 4
			                                                    : 3;
			break;
		...
		switch (effect) {
			case SE_CurrentHP: {
				...
				if (caster && caster->IsClient() && IsDetrimentalSpell(spell_id) && effect_value < 0) {
					...
					if (!caster->CastToClient()->IsFeigned() && (!GetOwner() || !GetOwner()->IsClient()))
...
void Mob::BuffFadeBySlot(int slot, bool iRecalcBonuses, bool message, bool update) {
	...
	if (buffs[slot].client) {
		...
		if (p && message && !buffs[slot].isdisc) {
			...
			else if ((p != this && !p->IsPet() && !IsBeneficialSpell(buffs[slot].spellid) && !IsCrowdControlSpell(buffs[slot].spellid)) || detrimental_exception) {
...
int16 Client::GetFocusEffect(focusType type, uint16 spell_id, std::string &item_name, bool dot_tick, int spell_level, bool include_items, bool include_spells, bool include_aa) {
	...
	if ((type == focusManaCost || type == focusImprovedHeal || type == focusImprovedDamage || type == focusSpellHateMod) && RuleB(Spells, LiveLikeFocusEffects)) {
	...
	*/
	...
	if (IsDamage || UseCastRestriction) {
		switch (value) {
			...
			case 304:
				if (IsClient() &&
				    ((GetClass() == WARRIOR) || (GetClass() == BARD) || (GetClass() == SHADOWKNIGHT) || (GetClass() == PALADIN) || (GetClass() == CLERIC) || (GetClass() == RANGER) || (GetClass() == SHAMAN) || (GetClass() == ROGUE)))
					return true;
				break;
