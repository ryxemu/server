   ...
   public:
	...
	uint32 GetDecayTime() {
		if (!corpse_decay_timer.Enabled())
			...
			return corpse_decay_timer.GetRemainingTime();
	}
	uint32 GetRezTime() {
		if (!corpse_rez_timer.Enabled())
			return 0;
		else
			return corpse_rez_timer.GetRemainingTime();
	}
	...
	void MoveToGraveyard() {
		if (IsPlayerCorpse()) corpse_graveyard_timer.Trigger();
	}
	void CalcCorpseName();
