...
void NPC::ResumeWandering() {
	...
	if (GetGrid() != 0) {
		...
		} else if (AIwalking_timer->Enabled()) {  // we are at a waypoint paused normally
...
void NPC::PauseWandering(int pausetime) {  // causes wandering to stop but is resumable
...
void NPC::MoveTo(const glm::vec4& position, bool saveguardspot, uint32 delay) {  // makes mob walk to specified location
...
bool ZoneDatabase::GetWaypoints(uint32 grid, uint16 zoneid, uint32 num, wplist* wp) {
	...
	std::string query = StringFormat(
	    "SELECT x, y, z, pause, heading FROM grid_entries "
	    "WHERE gridid = %i AND number = %i AND zoneid = %i",
	    grid, num, zoneid);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return false;
...
void ZoneDatabase::AddWP(Client* client, uint32 gridid, uint32 wpnum, const glm::vec4& position, uint32 pause, uint16 zoneid) {
	std::string query = StringFormat(
	    "INSERT INTO grid_entries (gridid, zoneid, `number`, x, y, z, pause, heading) "
...
void ZoneDatabase::DeleteWaypoint(Client* client, uint32 grid_num, uint32 wp_num, uint16 zoneid) {
	std::string query = StringFormat(
	    "DELETE FROM grid_entries WHERE "
...
int ZoneDatabase::GetHighestWaypoint(uint32 zoneid, uint32 gridid) {
	std::string query = StringFormat(
	    "SELECT COALESCE(MAX(number), 0) FROM grid_entries "
	    "WHERE zoneid = %i AND gridid = %i",
	    zoneid, gridid);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return 0;
...
int ZoneDatabase::GetRandomWaypointLocFromGrid(glm::vec4& loc, uint16 zoneid, int grid) {
	...
	std::string query = StringFormat(
	    ...
	    "FROM grid_entries WHERE `gridid` = %i AND `zoneid` = %u ORDER BY `number`",
	    grid, zone->GetZoneID());
