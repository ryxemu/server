...
Doors::Doors(const Door* door) : close_timer(5000),
                                 lift_timer(20000),
...
Doors::Doors(const char* dmodel, const glm::vec4& position, uint8 dopentype, uint16 dsize) : close_timer(5000),
                                                                                             lift_timer(20000),
                                                                                             m_Position(position),
...
bool Doors::DoorKeyCheck(Client* sender, uint32& key) {
	...
	if ((keyneeded == 0 && altkey == 0 && GetLockpick() == 0) ||
		...
		if (sender->GetGM()) {
			...
			return true;
		} else if (playerkey) {  // they have something they are trying to open it with
