...
void Mob::TryBashKickStun(Mob* defender, uint8 skill) {
	...
	if (!defender || defender->GetSpecialAbility(UNSTUNABLE) || defender->DivineAura() || defender->GetInvul() || (skill != EQ::skills::SkillBash && skill != EQ::skills::SkillKick && skill != EQ::skills::SkillDragonPunch)) {
		return;
	}
...
void Client::RangedAttack(Mob* other) {
	...
	if (other->IsNPC() && other->CastToNPC()->GetIgnoreDistance() > 349.0f && dist > (other->CastToNPC()->GetIgnoreDistance() * other->CastToNPC()->GetIgnoreDistance())) {
