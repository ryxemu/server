...
bool Mob::CastSpell(uint16 spell_id, uint16 target_id, CastingSlot slot,
	...
	if (IsClient() && spell_target && target_id != this->GetID() && spells[spell_id].targettype != ST_TargetOptional && spells[spell_id].targettype != ST_Self && spells[spell_id].targettype != ST_AECaster) {
		if (!zone->SkipLoS() && IsDetrimentalSpell(spell_id) && !spells[spell_id].npc_no_los && !CheckLosFN(spell_target, true) && (!IsHarmonySpell(spell_id) || spells[spell_id].targettype == ST_AETarget)  // harmony and wake of tranq require LoS
...
bool Client::CheckFizzle(uint16 spell_id) {
	...
	int spellCastingSkillTotalEffects =
	    +itembonuses.effective_casting_level_for_fizzle_check + spellbonuses.effective_casting_level_for_fizzle_check + aabonuses.effective_casting_level_for_fizzle_check;
	...
	if (spellFizzleAdjustment != 0 || spellCastingSkillTotalEffects < 0)  // something is hindering our casting, either a debuff or fizzle adjustment in the spell
	{
		...
		int chance = 0 + effectiveSpellCastingSkill             // + 235
		...
		if (GetClass() == BARD) {
			...
			cappedChance = cappedChance < 1 ? 1 : cappedChance > 95 ? 95
			                                                        : cappedChance;
		} else if (GetClass() <= PLAYER_CLASS_COUNT) {
			// min 5 max 95 for non bard
			cappedChance = cappedChance < 5 ? 5 : cappedChance > 95 ? 95
			                                                        : cappedChance;
		} else {
			// unknown class
			cappedChance = 0;
	...
	if (specializeSkill > 0) {
		...
		switch (spellCastingMasteryLevel) {
			case 1:
				spellCastingMasteryAdjustment = 2;
				break;
			case 2:
				spellCastingMasteryAdjustment = 5;
				break;
			case 3:
				spellCastingMasteryAdjustment = 10;
				break;
		}
...
void Mob::CastedSpellFinished(uint16 spell_id, uint32 target_id, CastingSlot slot,
	...
	if (IsClient() && ((slot == CastingSlot::Item)) && inventory_slot != 0xFFFFFFFF)  // 10 is an item
	{
...
bool Mob::DetermineSpellTargets(uint16 spell_id, Mob *&spell_target, Mob *&ae_center, CastAction_type &CastAction, bool isproc, CastingSlot slot) {
	...
	*/
	...
	if (IsClient() && spell_target != nullptr                                                                                      // null ptr crash safeguard
	    && spell_target->IsClient()                                                                                                // still self only if NPC targetted
	    && CastToClient()->CheckAAEffect(aaEffectProjectIllusion) && IsPlayerIllusionSpell(spell_id) && InSameGroup(spell_target)  // still self only if not grouped
	) {
	...
	switch (targetType) {
		...
		case ST_Undead: {
			if (!spell_target || (mob_body != BT_SummonedUndead && mob_body != BT_Undead && mob_body != BT_Vampire)) {
				// invalid target
		...
		case ST_Plant:
			if (target_bt == BT_Humanoid) target_bt = BT_Plant;
		case ST_UberDragon:
			if (target_bt == BT_Humanoid) target_bt = BT_VeliousDragon;
		case ST_UberGiant:
			if (target_bt == BT_Humanoid) target_bt = BT_BaneGiant;
		case ST_Animal:
			if (target_bt == BT_Humanoid) target_bt = BT_Animal;
			{
		...
		case ST_Corpse: {
			if (!spell_target || !spell_target->IsPlayerCorpse()) {
				...
				if (!spell_target)
					message = SPELL_NEED_TAR;
				else if (!spell_target->IsCorpse())
					message = ONLY_ON_CORPSES;
				else if (!spell_target->IsPlayerCorpse())
					message = CORPSE_NOT_VALID;
...
bool Mob::SpellFinished(uint16 spell_id, Mob *spell_target, CastingSlot slot, uint16 mana_used,
	...
	if (slot >= CastingSlot::Gem1 && slot < CastingSlot::MaxGems && GetClass() != BARD && GetClass() != RANGER && GetClass() != SHADOWKNIGHT && GetClass() != PALADIN && GetClass() != BEASTLORD) {
...
int Mob::CalcBuffDuration(Mob *caster, Mob *target, uint16 spell_id, int32 caster_level_override) {
	...
	if (caster == target && (target->aabonuses.IllusionPersistence || target->spellbonuses.IllusionPersistence || target->itembonuses.IllusionPersistence) &&
...
bool Mob::SpellOnTarget(uint16 spell_id, Mob *spelltar, bool reflect, bool use_resist_adjust, int16 resist_adjust, bool isproc, uint16 ae_caster_id, bool isrecourse, int recourse_spell_level) {
	...
	} else if (IsBeneficialSpell(spell_id) && !IsSummonPCSpell(spell_id) && !IsAEMemBlurSpell(spell_id) && !IsBindSightSpell(spell_id) && (!spelltar->IsPet() || spelltar->IsCharmedPet())  // no beneficial aggro for summoned pets
...
bool Client::SpellGlobalCheck(uint16 spell_ID, uint32 char_ID) {
	...
	std::string query = StringFormat(
	    "SELECT qglobal, value FROM spell_globals "
	    "WHERE spellid = %i",
	    spell_ID);
	...
	query = StringFormat(
	    "SELECT value FROM quest_globals "
