...
bool Mob::IsImmuneToMelee(Mob* attacker, int slot) {
	...
	if (weapon) {
		if (weapon->Magic || attacker->spellbonuses.MagicWeapon || attacker->itembonuses.MagicWeapon)
			magicWeapon = true;
	} else if ((attacker->GetClass() == MONK || attacker->GetClass() == BEASTLORD) && attacker->GetLevel() > 29 && (slot == EQ::invslot::slotPrimary || slot == EQ::invslot::slotSecondary || slot == EQ::invslot::slotHands))
		magicWeapon = true;
	...
	if (!magicWeapon && GetSpecialAbility(IMMUNE_MELEE_NONMAGICAL)) {
		if (attacker->IsNPC() && !attacker->GetSpecialAbility(SPECATK_MAGICAL) && attacker->GetLevel() < MAGIC_ATTACK_LEVEL) {
			...
			if ((attacker->GetClass() == MONK || attacker->GetClass() == BEASTLORD || attacker->GetClass() == BARD) && ((!weapon && (slot == EQ::invslot::slotPrimary || slot == EQ::invslot::slotSecondary)) || slot == EQ::invslot::slotHands)) {
	...
	if (GetSpecialAbility(IMMUNE_MELEE_EXCEPT_BANE)) {
		if (attacker->IsClient()) {
			...
			if (!attacker->GetSpecialAbility(SPECATK_BANE) && (!weapon || (weapon->BaneDmgBody != GetBodyType() && weapon->BaneDmgRace != GetRace())))
				return true;
		}
	}
...
bool Client::Attack(Mob* other, int hand, int damagePct) {
	...
	// SetAttackTimer();
	if (
	    (IsCasting() && GetClass() != BARD) || other == nullptr || ((IsClient() && CastToClient()->dead) || (other->IsClient() && other->CastToClient()->dead)) || (GetHP() < 0) || (!IsAttackAllowed(other))) {
	...
	if (other->IsImmuneToMelee(this, hand)) {
		...
		if (damage < 0 && (damage == DMG_DODGE || damage == DMG_PARRY || damage == DMG_RIPOSTE || damage == DMG_BLOCK) && aabonuses.StrikeThrough && zone->random.Roll(aabonuses.StrikeThrough)) {
...
void NPC::GiveExp(Client* give_exp_client, bool& xp) {
	...
	if (kr) {
		...
		for (int i = 0; i < MAX_RAID_MEMBERS; i++) {
			if (kr->members[i].member != nullptr && kr->members[i].member->IsClient() && IsOnHatelist(kr->members[i].member)) {  // If Group Member is Client
...
void Mob::AddToHateList(Mob* other, int32 hate, int32 damage, bool bFrenzy, bool iBuffTic, bool addpet) {
	...
	if (addpet && mypet) {  // I have a pet, add other to it
			...
			if (slot >= 0) {
				...
				if (spellbonuses.MitigateSpellRune[3] && (damage_to_reduce >= buffs[slot].magic_rune)) {
					Log(Logs::Detail, Logs::Spells,
					    "Mob::ReduceDamage SE_MitigateSpellDamage %d damage negated, %d"
					    " damage remaining, fading buff.",
					    damage_to_reduce, buffs[slot].magic_rune);
					damage -= buffs[slot].magic_rune;
					BuffFadeBySlot(slot);
				} else {
					Log(Logs::Detail, Logs::Spells,
					    "Mob::ReduceDamage SE_MitigateMeleeDamage %d damage negated, %d"
					    " damage remaining.",
					    damage_to_reduce, buffs[slot].magic_rune);
...
void Mob::DoRiposte(Mob* defender) {
	...
	if (defender->IsClient()) {
		if (defender->CastToClient()->IsUnconscious() || defender->IsStunned() || defender->CastToClient()->IsSitting() || defender->GetAppearance() == eaDead || defender->GetAppearance() == eaCrouching)
			return;
	}
...
void Client::SetAttackTimer(bool trigger) {
	...
	for (int i = EQ::invslot::slotRange; i <= EQ::invslot::slotSecondary; i++) {
		...
		if (ItemToUse != nullptr) {
			...
			if (ItemToUse->ItemClass != EQ::item::ItemClassCommon || ItemToUse->Damage == 0 || ItemToUse->Delay == 0) {
				// no weapon
				ItemToUse = nullptr;
			}
...
int Client::GetMitigation(bool ignoreCap, int item_ac_sum, int shield_ac, int spell_ac_sum, int classnum, int level, int base_race, int carried_weight, int agi, int defense_skill_value, int combat_stability_percent) {
	...
	if (playerClass == MONK) {
		...
		if (level < 15) {  // 1-14
			hardcap = 30;
			softcap = 14;
		} else if (level <= 29) {  // 15-29
			hardcap = 32;
			softcap = 15;
		} else if (level <= 44) {  // 30-44
			hardcap = 34;
			softcap = 16;
		} else if (level <= 50) {  // 45-50
			hardcap = 36;
			softcap = 17;
		} else if (level <= 54) {  // 51-54
			hardcap = 38;
			softcap = 18;
		} else if (level <= 59) {  // 55-59
			hardcap = 40;
			softcap = 20;
		} else if (level <= 61) {  // 60-61
			hardcap = 45;
			softcap = 24;
		} else if (level <= 63) {  // 62-63
			hardcap = 47;
			softcap = 24;
		} else if (level <= 64) {  // 64
			hardcap = 50;
