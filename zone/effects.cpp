	...
	if (spell->goodEffect != 0 && casttime > 3999 && spell->buffduration) {
		...
		if (SpellCastingDeftness_AA_Level > 0) {
			...
			switch (SpellCastingDeftness_AA_Level) {
				case 1:
					percent_mod = 95;
					break;  // Spell Casting Deftness 1 - 5% cast time reduction
				case 2:
					percent_mod = 85;
					break;  // Spell Casting Deftness 2 - 15% cast time reduction
				case 3:
					percent_mod = 75;
					break;  // Spell Casting Deftness 3 - 25% cast time reduction
			}
		...
		if (QuickBuff_AA_Level > 0) {
			...
			switch (QuickBuff_AA_Level) {
				case 1:
					percent_mod = 90;
					break;  // Quick Buff 1 - 10% cast time reduction
				case 2:
					percent_mod = 75;
					break;  // Quick Buff 2 - 25% cast time reduction
				case 3:
					percent_mod = 50;
					break;  // Quick Buff 3 - 50% cast time reduction
			}
			modified_cast_time = modified_cast_time * percent_mod / 100;
		}
	}
	...
	if (spell->goodEffect == 0 && casttime > 3999 && spell->buffduration == 0 && IsEffectInSpell(spell_id, SE_CurrentHP) && GetAA(aaSpellCastingFury) == 3) {
		...
		if (QuickDamage_AA_Level > 0) {
			...
			switch (QuickDamage_AA_Level) {
				case 1:
					percent_mod = 98;
					break;  // Quick Damage 1 - 2% cast time reduction
				case 2:
					percent_mod = 95;
					break;  // Quick Damage 2 - 5% cast time reduction
				case 3:
					percent_mod = 90;
					break;  // Quick Damage 3 - 10% cast time reduction
			}
			modified_cast_time = modified_cast_time * percent_mod / 100;
		}
	}
	...
	if (IsEffectInSpell(spell_id, SE_Succor)) {
		...
		if (QuickEvacuation_AA_Level > 0) {
			...
			switch (QuickEvacuation_AA_Level) {
				case 1:
					percent_mod = 90;
					break;  // Quick Evacuation 1 - 10% cast time reduction
				case 2:
					percent_mod = 75;
					break;  // Quick Evacuation 2 - 25% cast time reduction
				case 3:
					percent_mod = 50;
					break;  // Quick Evacuation 3 - 50% cast time reduction
			}
			modified_cast_time = modified_cast_time * percent_mod / 100;
		}
	}
		...
		if (IsEffectInSpell(spell_id, SE_SummonItem) || IsEffectInSpell(spell_id, SE_SummonPet) || spell_id == SPELL_MANIFEST_ELEMENTS || spell_id == SPELL_CALL_OF_THE_HERO) {
			...
			if (QuickSummoning_AA_Level > 0) {
				...
				switch (QuickSummoning_AA_Level) {
					case 1:
						percent_mod = 90;
						break;  // Quick Summoning 1 - 10% cast time reduction
					case 2:
						percent_mod = 75;
						break;  // Quick Summoning 2 - 25% cast time reduction
					case 3:
						percent_mod = 50;
						break;  // Quick Summoning 3 - 50% cast time reduction
				}
				modified_cast_time = modified_cast_time * percent_mod / 100;
			}
		}
...
void EntityList::AETaunt(Client *taunter, float range) {
	...
	while (it != npc_list.end()) {
		...
		if (zdiff < 25.0f  // our Z offets are much lower than Sony's, which could go up over 25 units.  Ours maxes ~12
		    && taunter->IsAttackAllowed(them) && DistanceSquaredNoZ(taunter->GetPosition(), them->GetPosition()) <= range) {
...
void EntityList::AEAttack(Mob *attacker, float dist, int targetLimit) {
	...
	for (auto it = mob_list.begin(); it != mob_list.end(); ++it) {
		...
		if (curmob->IsNPC() && curmob != attacker                                // this is not needed unless NPCs can use this
		    && (attacker->IsAttackAllowed(curmob)) && curmob->GetRace() != HORSE /* dont attack horses */
