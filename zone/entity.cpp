...
void EntityList::QueueCloseClients(Mob *sender, const EQApplicationPacket *app,
	...
	while (it != client_list.end()) {
		...
		if (ent != nullptr && (!ignore_sender || ent != sender) && (ent != SkipThisMob)) {
			...
			if (ent->Connected() &&
			    (filter == FilterNone || filter2 == FilterShow || (filter2 == FilterShowGroupOnly && (sender == ent || (ent->GetGroup() && ent->GetGroup()->IsGroupMember(sender)))) || (filter2 == FilterShowSelfOnly && ent == sender)) && (DistanceSquared(ent->GetPosition(), sender->GetPosition()) <= dist2)) {
...
void EntityList::QueueCloseClientsPrecalc(Mob *sender, const EQApplicationPacket *app, const EQApplicationPacket *app2,
	...
	while (it != client_list.end()) {
		...
		if ((!ignore_sender || ent != sender) && (ent != SkipThisMob)) {
			...
			} else if (app2 != nullptr && !ent->GetInside(sender->GetID()) && !ent->SameLastPosition(sender->GetID(), sender->GetPosition()) && ent->GetGM()) {
...
void EntityList::AddHealAggro(Mob *target, Mob *caster, uint16 hate) {
	...
	while (it != npc_list.end()) {
		...
		if (cur->IsPet() || !cur->CheckAggro(target) || cur->IsFeared() || (target->IsClient() && target->CastToClient()->IsFeigned() && !cur->GetSpecialAbility(IMMUNE_FEIGN_DEATH))) {
			++it;
			continue;
		}
...
void EntityList::AddLootToNPCS(uint32 item_id, uint32 count) {
	...
	auto it = npc_list.begin();
	while (it != npc_list.end()) {
		if (!it->second->IsPet() && !it->second->IsUnTargetable())
			npc_count++;
		++it;
	}
