...
void NPC::UpdateEquipmentLight() {
	...
	for (auto iter = itemlist.begin(); iter != itemlist.end(); ++iter) {
		...
		if (m_Light.Type[EQ::lightsource::LightEquipment] != 0 && (item->Light < 9 || item->Light > 13)) {
			continue;
		}
...
uint32 ZoneDatabase::CreateNewNPCCommand(const char* zone, Client* client, NPC* spawn, uint32 extra) {
	...
	if (npc_type_id) {
		query = StringFormat(
		    "INSERT INTO npc_types (id, name, level, race, class, hp, gender, "
	...
	query = StringFormat(
	    "INSERT INTO spawn2 (zone, x, y, z, respawntime, heading, spawngroupID) "
...
uint32 ZoneDatabase::UpdateNPCTypeAppearance(Client* client, NPC* spawn) {
	std::string query = StringFormat(
	    "UPDATE npc_types SET name = \"%s\", level = %i, race = %i, class = %i, "
...
uint32 ZoneDatabase::DeleteSpawnLeaveInNPCTypeTable(const char* zone, Client* client, NPC* spawn) {
	...
	std::string query = StringFormat(
	    "SELECT id, spawngroupID FROM spawn2 WHERE "
...
uint32 ZoneDatabase::DeleteSpawnRemoveFromNPCTypeTable(const char* zone, Client* client, NPC* spawn) {
	...
	std::string query = StringFormat(
	    "SELECT id, spawngroupID FROM spawn2 WHERE zone = '%s' "
	    "AND spawngroupID = %i",
	    zone, spawn->GetSp2());
	auto results = QueryDatabase(query);
	if (!results.Success())
...
void NPC::ModifyNPCStat(const char* identifier, const char* newValue) {
	...
	if (id == "ac") {
		...
		ModifySpecialAbility(val.c_str());
		return;
	} else if (id == "atk") {
		ATK = atoi(val.c_str());
		return;
		...
		return;
	} else if (id == "see_sneak") {
		see_sneak = atoi(val.c_str());
		return;
	...
	if (zone->zonemap->LineIntersectsZone(currentLoc, newLoc, 0.0f, &hitLocation, &hitNormal, &hitDistance)) {
		...
		if (hitNormal.x != 0.0f || hitNormal.y != 0.0f) {
			...
			{                                             // weird stuff like pushing through hills or walls
				...
				closestWallNormal.x = hitNormal.x;
				closestWallNormal.y = hitNormal.y;
				closestWallNormal.z = hitNormal.z;
				closestHitLocation.x = hitLocation.x;
				closestHitLocation.y = hitLocation.y;
				closestHitLocation.z = hitLocation.z;
				closestPoint = 0;
