...
class NPC : public Mob {
   public:
	...
	virtual uint16 GetSkill(EQ::skills::SkillType skill_num) const {
	...
	void AddHate(Mob* other, int32 hate = 0, int32 damage = 0, bool bFrenzy = false, bool iAddIfNotExist = true) {
		hate_list.Add(other, hate, damage, bFrenzy, iAddIfNotExist);
		SetAssisting(false);
	}
	...
	void RestoreNPCFactionID() {
		npc_faction_id = precharm_npc_faction_id;
		database.GetFactionIdsForNPC(npc_faction_id, &faction_list, &primary_faction);
	}
	...
	bool IsWalled() {
		if (wall_normal1_x || wall_normal1_y || wall_normal2_x || wall_normal2_y)
			return true;
		else
			return false;
	};
	...
	inline void SetClass(uint8 classNum) {
		if (classNum <= PLAYER_CLASS_COUNT && classNum > 0) class_ = static_cast<uint8>(classNum);
	};  // for custom scripts
	...
	inline void SetCastRateBeneficial(uint8 rate) {
		AISpellVar.engaged_beneficial_other_chance = rate;
		if (AISpellVar.engaged_beneficial_self_chance < rate) AISpellVar.engaged_beneficial_self_chance = rate;
	}
