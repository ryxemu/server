...
Mob* EntityList::GetTrapTrigger(Trap* trap) {
	...
	for (auto it = client_list.begin(); it != client_list.end(); ++it) {
		...
		if ((diff.x * diff.x + diff.y * diff.y) <= maxdist && diff.z <= trap->maxzdiff) {
...
bool ZoneDatabase::LoadTraps(const char* zonename) {
	std::string query = StringFormat(
	    ...
	    "`group`, triggered_number, despawn_when_triggered, undetectable FROM traps WHERE zone='%s'",
	    zonename);
...
bool ZoneDatabase::SetTrapData(Trap* trap, bool repopnow) {
	...
	if (trap->group > 0) {
		query = StringFormat(
		    "SELECT id, x, y, z, effect, effectvalue, effectvalue2, skill, "
		    "maxzdiff, radius, chance, message, respawn_time, respawn_var, level, "
		    "triggered_number, despawn_when_triggered, undetectable FROM traps WHERE zone='%s' AND `group`=%d AND id != %d ORDER BY RAND() LIMIT 1",
		    zone->GetShortName(), trap->group, dbid);
	} else {
		...
		query = StringFormat(
		    "SELECT id, x, y, z, effect, effectvalue, effectvalue2, skill, "
		    "maxzdiff, radius, chance, message, respawn_time, respawn_var, level, "
		    "triggered_number, despawn_when_triggered, undetectable FROM traps WHERE zone='%s' AND id = %d",
		    zone->GetShortName(), dbid);
	}
