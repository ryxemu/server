...
bool ZoneDatabase::logevents(const char* accountname, uint32 accountid, uint8 status, const char* charname, const char* target, const char* descriptiontype, const char* description, int event_nid) {
	...
	std::string query = StringFormat(
	    "INSERT INTO eventlog (accountname, accountid, status, "
...
void ZoneDatabase::UpdateFeedback(Feedback_Struct* feedback) {
	...
	std::string query = StringFormat(
	    "INSERT INTO feedback (name, message, zone, date) "
...
void ZoneDatabase::AddSoulMark(uint32 charid, const char* charname, const char* accname, const char* gmname, const char* gmacctname, uint32 utime, uint32 type, const char* desc) {
	std::string query = StringFormat(
	    "INSERT INTO character_soulmarks (charid, charname, acctname, gmname, gmacctname, utime, type, `desc`) "
	    "VALUES(%i, '%s', '%s','%s','%s', %i, %i, '%s')",
	    charid, charname, accname, gmname, gmacctname, utime, type, desc);
	auto results = QueryDatabase(query);
	if (!results.Success())
...
void ZoneDatabase::GetEventLogs(const char* name, char* target, uint32 account_id, uint8 eventid, char* detail, char* timestamp, CharacterEventLog_Struct* cel) {
	...
	std::string query = StringFormat(
	    "SELECT id, accountname, accountid, status, charname, target, "
	    "time, descriptiontype, description FROM eventlog WHERE %s",
	    modifications);
	auto results = QueryDatabase(query);
	if (!results.Success())
		return;
...
void ZoneDatabase::LoadWorldContainer(uint32 parentid, EQ::ItemInstance* container) {
	...
	std::string query = StringFormat(
	    "SELECT bagidx, itemid, charges "
	    "FROM object_contents WHERE parentid = %i",
	    parentid);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
...
EQ::ItemInstance* ZoneDatabase::LoadSingleTraderItem(uint32 CharID, int16 islotid, uint16 slotid) {
	std::string query = StringFormat(
	    "SELECT * FROM trader WHERE char_id = %i AND slot_id = %i AND i_slotid = %i"
	    " ORDER BY slot_id LIMIT 80",
	    CharID, slotid, islotid);
	auto results = QueryDatabase(query);
	if (!results.Success())
		return nullptr;

	if (results.RowCount() == 0) {
		Log(Logs::Detail, Logs::Bazaar, "LoadSingleTraderItem: No items found.");
		fflush(stdout);
		return nullptr;
	}
...
bool ZoneDatabase::LoadCharacterData(uint32 character_id, PlayerProfile_Struct* pp, ExtendedProfile_Struct* m_epp) {
	...
	int r = 0;
	for (auto& row = results.begin(); row != results.end(); ++row) {
		strcpy(pp->name, row[r]);
		r++;  // "`name`,                    "
		strcpy(pp->last_name, row[r]);
		r++;  // "last_name,                 "
		pp->gender = atoi(row[r]);
		r++;  // "gender,                    "
		pp->race = atoi(row[r]);
		r++;  // "race,                      "
		pp->class_ = atoi(row[r]);
		r++;  // "class,                     "
		pp->level = atoi(row[r]);
		r++;  // "`level`,                   "
		pp->deity = atoi(row[r]);
		r++;  // "deity,                     "
		pp->birthday = atoi(row[r]);
		r++;  // "birthday,                  "
		pp->lastlogin = atoi(row[r]);
		r++;  // "last_login,                "
		pp->timePlayedMin = atoi(row[r]);
		r++;  // "time_played,               "
		pp->pvp = atoi(row[r]);
		r++;  // "pvp_status,                "
		pp->level2 = atoi(row[r]);
		r++;  // "level2,                    "
		pp->anon = atoi(row[r]);
		r++;  // "anon,                      "
		pp->gm = atoi(row[r]);
		r++;  // "gm,                        "
		pp->intoxication = atoi(row[r]);
		r++;  // "intoxication,              "
		pp->haircolor = atoi(row[r]);
		r++;  // "hair_color,                "
		pp->beardcolor = atoi(row[r]);
		r++;  // "beard_color,               "
		pp->eyecolor1 = atoi(row[r]);
		r++;  // "eye_color_1,               "
		pp->eyecolor2 = atoi(row[r]);
		r++;  // "eye_color_2,               "
		pp->hairstyle = atoi(row[r]);
		r++;  // "hair_style,                "
		pp->beard = atoi(row[r]);
		r++;  // "beard,                     "
		strcpy(pp->title, row[r]);
		r++;  // "title,                     "
		strcpy(pp->suffix, row[r]);
		r++;  // "suffix,                    "
		pp->exp = atoi(row[r]);
		r++;  // "exp,                       "
		pp->points = atoi(row[r]);
		r++;  // "points,                    "
		pp->mana = atoi(row[r]);
		r++;  // "mana,                      "
		pp->cur_hp = atoi(row[r]);
		r++;  // "cur_hp,                    "
		pp->STR = atoi(row[r]);
		r++;  // "str,                       "
		pp->STA = atoi(row[r]);
		...
		pp->z = atof(row[r]);
		r++;  // "z,                         "
		pp->heading = atof(row[r]);
		r++;  // "heading,                   "
		pp->autosplit = atoi(row[r]);
		r++;  // "autosplit_enabled,         "
		pp->zone_change_count = atoi(row[r]);
		r++;  // "zone_change_count,         "
		pp->hunger_level = atoi(row[r]);
		r++;  // "hunger_level,              "
		pp->thirst_level = atoi(row[r]);
		r++;  // "thirst_level,              "
		pp->zone_id = atoi(row[r]);
		r++;  // "zone_id,                   "
		pp->air_remaining = atoi(row[r]);
		r++;  // "air_remaining,             "
		pp->aapoints_spent = atoi(row[r]);
		r++;  // "aa_points_spent,           "
		pp->expAA = atoi(row[r]);
		r++;  // "aa_exp,                    "
		pp->aapoints = atoi(row[r]);
		r++;  // "aa_points,                 "
		pp->boatid = atoi(row[r]);
		r++;  // "boatid,					"
		strncpy(pp->boat, row[r], 32);
		r++;  // "boatname					"
		pp->showhelm = atobool(row[r]);
		r++;  // "showhelm,					"
		pp->fatigue = atoi(row[r]);
		r++;  // "fatigue,					"
		m_epp->aa_effects = atoi(row[r]);
		r++;  // "`e_aa_effects`,			"
		m_epp->perAA = atoi(row[r]);
		r++;  // "`e_percent_to_aa`,			"
		m_epp->expended_aa = atoi(row[r]);
		r++;  // "`e_expended_aa_spent`,		"
	}
	return true;
}
...
bool ZoneDatabase::SaveCharacterLanguage(uint32 character_id, uint32 lang_id, uint32 value) {
	std::string query = StringFormat("REPLACE INTO `character_languages` (id, lang_id, value) VALUES (%u, %u, %u)", character_id, lang_id, value);
	QueryDatabase(query);
...
bool ZoneDatabase::SaveCharacterSkill(uint32 character_id, uint32 skill_id, uint32 value) {
	std::string query = StringFormat("REPLACE INTO `character_skills` (id, skill_id, value) VALUES (%u, %u, %u)", character_id, skill_id, value);
	auto results = QueryDatabase(query);
...
bool ZoneDatabase::SaveCharacterAA(uint32 character_id, uint32 aa_id, uint32 current_level) {
	std::string rquery = StringFormat(
	    "REPLACE INTO `character_alternate_abilities` (id, aa_id, aa_value)"
	    " VALUES (%u, %u, %u)",
...
void ZoneDatabase::SaveMerchantTemp(uint32 npcid, uint32 slot, uint32 item, uint32 charges, uint32 quantity) {
	std::string query = StringFormat(
	    "REPLACE INTO merchantlist_temp (npcid, slot, itemid, charges, quantity) "
	    "VALUES(%d, %d, %d, %d, %d)",
	    npcid, slot, item, charges, quantity);
	QueryDatabase(query);
}
...
bool ZoneDatabase::UpdateZoneSafeCoords(const char* zonename, const glm::vec3& location) {
	std::string query = StringFormat(
	    "UPDATE zone SET safe_x='%f', safe_y='%f', safe_z='%f' "
	    "WHERE short_name='%s';",
...
uint8 ZoneDatabase::RaidGroupCount(uint32 raidid, uint32 groupid) {
	std::string query = StringFormat(
	    "SELECT count(charid) FROM raid_members "
	    "WHERE raidid = %d AND groupid = %d;",
	    raidid, groupid);
	auto results = QueryDatabase(query);
...
bool ZoneDatabase::LoadBlockedSpells(int32 blockedSpellsCount, ZoneSpellsBlocked* into, uint32 zoneid) {
	...
	std::string query = StringFormat(
	    "SELECT id, spellid, type, x, y, z, x_diff, y_diff, z_diff, message "
	    "FROM blocked_spells WHERE zoneid = %d ORDER BY id ASC",
	    zoneid);
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return false;
...
void ZoneDatabase::LoadBuffs(Client* client) {
	...
	std::string query = StringFormat(
	    "SELECT spell_id, slot_id, caster_level, caster_name, ticsremaining, "
	    "counters, melee_rune, magic_rune, persistent, "
	    "ExtraDIChance, bard_modifier "
	    "FROM `character_buffs` WHERE `id` = '%u'",
	    client->CharacterID());
	auto results = QueryDatabase(query);
	if (!results.Success()) {
		return;
...
void ZoneDatabase::RemoveTempFactions(Client* client) {
	std::string query = StringFormat(
	    "DELETE FROM character_faction_values "
...
void ZoneDatabase::LoadPetInfo(Client* client) {
	...
	std::string query = StringFormat(
	    "SELECT `pet`, `petname`, `petpower`, `spell_id`, "
...
bool ZoneDatabase::SetCharacterFactionLevel(uint32 char_id, int32 faction_id, int32 value, uint8 temp, faction_map& val_list) {
	...
	query = StringFormat(
	    "INSERT INTO `character_faction_values` "
...
bool ZoneDatabase::UpdateCharacterCorpseBackup(uint32 db_id, uint32 char_id, const char* char_name, uint32 zone_id, PlayerCorpse_Struct* dbpc, const glm::vec4& position, bool is_rezzed) {
	std::string query = StringFormat(
	    "UPDATE `character_corpses_backup` SET \n"
	    "`charname` =		  '%s',\n"
	    "`charid` =				%d,\n"
	    "`exp` =                 %u,\n"
	    "`gmexp` =				%u,\n"
...
uint32 ZoneDatabase::SaveCharacterCorpse(uint32 charid, const char* charname, uint32 zoneid, PlayerCorpse_Struct* dbpc, const glm::vec4& position) {
	...
	std::string query = StringFormat(
	    "INSERT INTO `character_corpses` SET \n"
	    "`charname` =		  '%s',\n"
	    "`zone_id` =				%u,\n"
	    "`charid` =				%d,\n"
	    "`x` =					%1.1f,\n"
	...
	if (dbpc->itemcount == 0) {
		...
		for (unsigned int i = 0; i < dbpc->itemcount; i++) {
			if (first_entry != 1) {
				corpse_items_query = StringFormat(
...
bool ZoneDatabase::SaveCharacterCorpseBackup(uint32 corpse_id, uint32 charid, const char* charname, uint32 zoneid, PlayerCorpse_Struct* dbpc, const glm::vec4& position) {
	/* Dump Basic Corpse Data */
	std::string query = StringFormat(
	    "INSERT INTO `character_corpses_backup` SET \n"
	    "`id` =						%u,\n"
	    "`charname` =		  '%s',\n"
	    "`zone_id` =				%u,\n"
	    "`charid` =				%d,\n"
	...
	for (unsigned int i = 0; i < dbpc->itemcount; i++) {
		if (first_entry != 1) {
			query = StringFormat(
			    "REPLACE INTO `character_corpse_items_backup` \n"
			    " (corpse_id, equip_slot, item_id, charges) \n"
			    " VALUES (%u, %u, %u, %u) \n",
			    corpse_id,
			    dbpc->items[i].equip_slot,
...
Corpse* ZoneDatabase::SummonBuriedCharacterCorpses(uint32 char_id, uint32 dest_zone_id, const glm::vec4& position) {
	...
	std::string query = StringFormat(
	    "SELECT `id`, `charname`, UNIX_TIMESTAMP(time_of_death), `is_rezzed` "
...
bool ZoneDatabase::SaveSoulboundItems(Client* client, std::list<EQ::ItemInstance*>::const_iterator& start, std::list<EQ::ItemInstance*>::const_iterator& end) {
	// Delete cursor items
	std::string query = StringFormat(
	    "DELETE FROM character_inventory WHERE id = %i "
...
bool ZoneDatabase::UpdateSkillDifficulty(uint16 skillid, float difficulty) {
	std::string query = StringFormat(
	    "UPDATE skill_difficulty SET difficulty = %0.2f "
	    "WHERE skillid = %u;",
	    difficulty, skillid);
	auto results = QueryDatabase(query);
	if (!results.Success())
