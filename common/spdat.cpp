...
bool IsCureSpell(uint16 spell_id) {
	...
	for (int i = 0; i < EFFECT_COUNT; i++) {
		if (sp.effectid[i] == SE_DiseaseCounter || sp.effectid[i] == SE_PoisonCounter || sp.effectid[i] == SE_CurseCounter)
			CureEffect = true;
	}
...
bool IsNeutralSpell(uint16 spell_id) {
	...
	return (IsBindSightSpell(spell_id) || (IsMemBlurSpell(spell_id) && !IsMezSpell(spell_id)) || IsLuclinPortSpell(spell_id) || IsHarmonySpell(spell_id) || spells[spell_id].effectid[0] == SE_Teleport2);  // some PoP NPC teleports are flagged beneficial
}
