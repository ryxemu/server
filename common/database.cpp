...
bool Database::DeleteCharacter(char* name) {
	...
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_inventory` WHERE `id` = '%d'", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_faction_values` WHERE `id` = '%d'", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_data` WHERE `id` = '%d'", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_skills` WHERE `id` = %u", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_languages` WHERE `id` = %u", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_bind` WHERE `id` = %u", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_alternate_abilities` WHERE `id` = %u", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_currency` WHERE `id` = %u", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_data` WHERE `id` = %u", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_spells` WHERE `id` = %u", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_memmed_spells` WHERE `id` = %u", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_inspect_messages` WHERE `id` = %u", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_zone_flags` WHERE `id` = '%d'", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_keyring` WHERE `id` = '%d'", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_buffs` WHERE `id` = '%d'", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_consent` WHERE `name` = '%s'", name);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_soulmarks` WHERE `id` = '%d'", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_timers` WHERE `id` = '%d'", charid);
	QueryDatabase(query);
...
void Database::DeleteCharacterCorpses(uint32 charid) {
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		std::string delete_query = StringFormat("DELETE FROM `character_corpse_items` WHERE `corpse_id` = '%d'", corpseid);
		QueryDatabase(delete_query);
		delete_query = StringFormat("DELETE FROM `character_corpse_items_backup` WHERE `corpse_id` = '%d'", corpseid);
		QueryDatabase(delete_query);
	}

	query = StringFormat("DELETE FROM `character_corpses` WHERE `charid` = '%d'", charid);
	QueryDatabase(query);
	query = StringFormat("DELETE FROM `character_corpses_backup` WHERE `charid` = '%d'", charid);
	QueryDatabase(query);
}
...
bool Database::SaveCharacterCreate(uint32 character_id, uint32 account_id, PlayerProfile_Struct* pp) {
	...
	/* Save Bind Points */
	query = StringFormat(
	    "REPLACE INTO `character_bind` (id, zone_id, x, y, z, heading, is_home)"
